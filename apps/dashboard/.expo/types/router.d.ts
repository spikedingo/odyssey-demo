...
declare module 'expo-router' {
  export namespace ExpoRouter {
    export interface __routes<T extends string = string> extends Record<string, unknown> {
      StaticRoutes: `/` | `/(dashboard)` | `/(dashboard)/crm` | `/(dashboard)/home` | `/(dashboard)/menu` | `/(dashboard)/orders` | `/(dashboard)/settings` | `/_sitemap` | `/crm` | `/home` | `/menu` | `/orders` | `/settings` | `/ui-library`;
